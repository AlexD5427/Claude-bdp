...
function AltaExpediente({
  ...
  const hayDatos = form.identificador.trim() !== "" || form.nombre.trim() !== "";
  ...
  return (
    <Lateral
      ...
      ancho="max-w-2xl"
      bloqueado={guardando}
      confirmarCierre={hayDatos ? "Hay datos escritos en el formulario. ¿Cerrar y perderlos?" : undefined}
      pie={
    ...
    >
      <div className="space-y-4">
        <Panel titulo="Datos generales">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Campo etiqueta="Agencia" ayuda="Se toma del catálogo Auxiliar; puedes escribir una nueva.">
              <Entrada list="doc-agencias" value={form.agencia} onChange={(e) => poner("agencia", e.target.value)} />
            ...
            <Campo etiqueta="Gerencia">
              <Entrada list="doc-gerencias" value={form.gerencia} onChange={(e) => poner("gerencia", e.target.value)} />
        ...
        <Panel titulo="Clasificación" descripcion="Determina qué requisitos se le van a exigir.">
          ...
          {tipoElegido && <p className="doc-prose mt-2 text-[11px] text-[color:var(--doc-text-faint)]">{tipoElegido.descripcion}</p>}

          {rama?.habilitada ? (
            <div className="doc-sunken mt-3 p-3">
              <p className="text-xs font-semibold text-[color:var(--doc-text)]">
                {rama.total} requisitos aplicables · {rama.obligatorios} obligatorios
              </p>
              <p className="doc-prose mt-1 text-[11px] leading-relaxed text-[color:var(--doc-text-muted)]">
                {rama.codigos
                  .slice(0, 8)
