/**
 * docSchema.ts - la unica fuente de verdad del modulo de Documentacion.
 *
 * Aqui viven los tipos, el catalogo de columnas del libro y la semantica de los
 * colores. Es el espejo en TypeScript de lo que `00_Manifest.gs` declara en Apps
 * Script.
 *
 * Estan duplicados a proposito, y conviene explicar por que: gracias a esta
 * copia el frontend puede mostrar exactamente lo que se va a escribir en la hoja
 * -- que columna, que texto, que color -- sin ir a preguntarselo al servidor.
 * La alternativa seria una llamada de red para pintar una fila, lo que dejaria
 * la interfaz muda mientras no hay conexion. El precio es mantener los dos lados
 * sincronizados; `10_Tests.gs` comprueba el lado servidor y `docSchema.test.ts`
 * el lado cliente contra las mismas cifras.
 */
...
/** Orden de recorrido al pulsar repetidamente sobre el estado de un documento. */
export const DOC_STATUS_CYCLE: DocStatus[] = [
	"pendiente",
	"presentado",
	"observado",
	"no_aplica",
...
export type Dossier = {
	...
	/** Campos que solo existen en la hoja y no tienen equivalente en la ficha. */
	tipoEmpleado?: string
	responsable?: string
	observacion?: string
	updatedAt?: string
	updatedBy?: string
	/** Fila historica del Excel original, sin checklist detras. */
	heredada?: boolean
}
...
/* ---------------------- Vocabulario del libro de calculo ------------------ */

/**
 * Los cuatro valores que el area escribe en las columnas de documentos.
 *
 * El guion bajo no es un hueco por descuido: en el libro original significa
 * "revisado y no corresponde pedirlo", que no es lo mismo que una celda vacia
 * (nadie lo ha mirado todavia). Esa distincion se respeta.
 */
...
/** Colores tomados del libro real. No son decorativos: cada uno significa algo. */
...
export type DocColumnSpec = {
	clave: string
	encabezado: string
	grupo: "base" | "documento" | "gestion"
	/** Ids del catálogo que alimentan esta columna cuando no se escribe a mano. */
	items?: string[]
...
/**
 * Columnas A-W tal como estan en `registro_ingresos.xlsx`.
 *
 * Se conservan las rarezas del original a proposito: el espacio final de
 * "Tipo de Empleado ", el salto de linea del consentimiento y las dos columnas
 * llamadas "CONTRATO DE FIANZA". Cambiarlas seria mas limpio y rompería las
 * formulas y los filtros que el area ya tiene montados sobre esos nombres.
 */
...
/** Columnas que anade el sistema a partir de la X. */
...
export const DOC_TABLE_COLUMNS = [
 ...
 * mano, el almacenamiento local puede venir de una version anterior del modulo.
 * Nada que salga de aqui deberia poder romper la interfaz.
 */
export function normalizeDossier(raw: unknown, fallbackGroup: DocGroup = "personal"): Dossier | null {
	...
	for (const it of itemsRaw) {
		...
		items.push({
			...
			observation: toText(o.observation) || undefined,
...
/** Ano al que pertenece un expediente, para saber en que pestana va. */
...
/* ------------------- Previsualizacion de la fila del libro ---------------- */

/**
 * Calcula que se escribira en cada columna del libro.
 *
 * Replica `docSheetValuesFor_` del backend para que la vista previa de la hoja
 * sea fiel sin llamadas de red. Regla identica en los dos lados: lo que alguien
 * escribio a mano en la hoja nunca se pisa con un valor derivado.
 */
export function sheetValuesFor(dossier: Dossier): Record<string, string> {
	...
	for (const columna of DOC_BASE_COLUMNS) {
		...
		if (columna.manual || !columna.items?.length) {
...
/** Color de fila que tendra el expediente en el libro. */
